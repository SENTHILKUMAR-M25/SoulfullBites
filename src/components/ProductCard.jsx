...
export default function ProductCard({ product }) {
  ...
  return (
    <div className="bg-[#1a1a1a] rounded-2xl overflow-hidden shadow-lg border border-[#4B2E2B] hover:shadow-2xl transition duration-300 hover:-translate-y-1">
      ...
      <div className="p-4">
        ...
        <h3 className="font-semibold text-lg text-[#F5E6C8]">
          {product.name}
        </h3>
        ...
        {product.price && (
          <button
            onClick={() => addToCart(product, "default", product.price)}
            className="mt-4 w-full bg-[#C9A227] hover:bg-[#d4af37] text-black py-2 rounded-lg font-semibold transition"
          >
            Add ₹{product.price}
          </button>
        )}
        ...
        {product.prices &&
          Object.entries(product.prices).map(([key, value]) => (
            <button
              key={key}
              onClick={() => addToCart(product, key, value)}
              className="mt-2 w-full border border-[#C9A227] text-[#C9A227] hover:bg-[#4B2E2B] hover:text-[#F5E6C8] py-2 rounded-lg font-medium transition"
            >
              {key} ₹{value}
            </button>
          ))}
